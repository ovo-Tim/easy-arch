"""
 Base on https://github.com/peakwinter/python-pacman
"""
...
def install(packages, needed=True):
    ...
    needed <bool>
    """
    s = pacman("-S", packages, ["--needed" if needed else None], print_r = True)
    if s["code"] != 0:
...
def remove(packages, purge=False):
    ...
    purge <bool>
    """
    s = pacman("-Rc{0}".format("n" if purge else ""), packages, print_r=True)
    if s["code"] != 0:
...
def pacman(flags, pkgs=[], eflgs=[], pacman_bin=None, print_r = False):
    """Subprocess wrapper, get all data"""
    if pacman_bin is None:
        pacman_bin = __PACMAN_BIN
    if 'pacman' in pacman_bin:
        cmd = ['sudo']
    else:
        cmd = []
    if not pkgs:
